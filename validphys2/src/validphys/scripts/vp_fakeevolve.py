...
def process_args():
    ...
    parser.add_argument(
        "fixed_simunet_fit", help="Name of the BSM-parameter only simunet fit."
    )
...
def main():
    ...
    for i in range(int(args.num_reps)):
        source_file = (
            fixed_fit_dir / "postfit" / ("replica_" + str(i + 1)) / (fixed_fit + ".dat")
        )
        destination_file = (
            input_dir / "nnfit" / ("replica_" + str(i + 1)) / (input_fit + ".dat")
        )
    ...
    shutil.copy(
        fixed_fit_dir / "nnfit" / (fixed_fit + ".info"),
        input_dir / "nnfit" / (input_fit + ".info"),
    )
